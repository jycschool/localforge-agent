...
| `read_file` | `path`, `startLine`, `endLine` | 带行号内容、总行数、完整文件 SHA-256 | 自动，只读 |
| `replace_in_file` | `path`, `oldText`, `newText` | 修改统计、前后摘要 | 自动，项目内写入 |
| `edit_file_lines` | `path`, `startLine`, `endLine`, `expectedHash`, `replacement` | 行范围、前后摘要 | 自动，项目内写入；摘要不匹配则拒绝 |
| `write_file` | `path`, `content` | 创建/覆盖结果、前后摘要 | 自动，项目内写入 |
...
文件路径先做词法范围检查，再对既有目标或最近既有父目录做 `realpath` 检查。符号链接被跳过；依赖、缓存和构建目录不会进入默认遍历。列表与搜索达到结果上限后立即停止递归，遍历每层目录和文件前检查 AbortSignal，避免大仓库取消后继续空扫。`read_file` 返回完整文件摘要；`edit_file_lines` 只在摘要仍匹配时替换指定行，外部修改会产生可恢复的 `STALE_FILE_CONTENT`，从而避免弱模型用过期行号覆盖新内容。命令 cwd 固定为项目根目录，子进程环境会剥离常见 Key、Token、Secret、Password、Credential 和 Auth 变量，输出保留长度受配置限制。命令工具分别记录人工批准等待与本地进程执行耗时，AgentLoop 的整次工具调用耗时仍可用于总链路分析，两者不能混为性能数据。
...
设置中的权限不是装饰字段。`readOnly` 只把 `list_files`、`search_text`、`read_file` 注册给本次 ToolRegistry；`workspace` 才包含精确替换、按行编辑、完整写入和命令工具，命令仍逐次审批。响应档位分别把兼容接口的 `max_tokens` 设为 4,096、8,192、16,384，并同步约束 Agent 的探索风格；该字段描述单轮输出预算与任务深度，不宣称改变服务端每秒生成速度。
...
| 工具参数或执行无效 | 返回错误码、是否可重试、恢复建议和必要细节；连续 3 次完全相同的失败调用仍立即熔断 |
| 交替读取、搜索或同类失败没有进展 | 最近八次动作检测重复签名和同类错误；第一次注入纠偏指令，未发生写入、成功命令或计划推进而再次循环时提前停止 |
| 直接模式修改后提前结束 | `DirectCompletionPolicy` 追踪成功写入及其后的读取/命令；首次未验证完成会阻止结束并要求验证，仍无法验证时允许下一次答复如实报告风险，避免死循环 |
| 接近步骤上限 | 剩余三次模型调用时注入收尾预算提示，优先解决未完成事项与最小验证；用户设置的硬上限不被静默扩大 |
...
| 可测试性 | 核心服务可脱离 Electron 测试；桌面边界有 IPC、结构和状态合同 | 33 个测试文件、157 项测试；桌面专项 33 项 |
