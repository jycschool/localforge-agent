...
LocalForge 的基础循环是显式的 ReAct 变体，但不要求或保存模型私有思考链：模型依据消息与工具结果选择下一项可观察动作，ToolRegistry 执行后把结构化结果按 call id 回传，下一轮再观察并决策。界面只展示简短说明、工具、结果、计划和证据；`reasoning_content` 不写入历史或报告。
