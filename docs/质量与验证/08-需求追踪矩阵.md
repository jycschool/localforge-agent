...
| NFR-08 可测试 | 核心模块与 Preload/主进程 IPC/配置/工作区状态/快速打开/草稿/富文本/路径脱敏边界可隔离验证；桌面结构与状态有独立合同门禁 | 当前 157 项自动化测试，其中桌面专项 33 项 |
...
| 验证 | 候选构建 | 157 项测试、桌面专项门禁、Windows 自动化门禁、真实模型、连续操作检查、交付脚本 | 无阻塞缺陷、连续操作状态一致 | 仓库侧已完成 |
| 发布 | 验证通过的提交 | Windows 可运行制品、标签 Release、视频、README.txt、最终推送和压缩包 | 自动制品可下载；最终提交冻结；课程材料人工复核 | 仓库与 README 已完成，视频/压缩包待提交者复看上传 |
