...
| NFR-08 可测试 | 核心模块与 Preload/主进程 IPC/配置/工作区状态/快速打开/草稿/富文本/路径脱敏边界可隔离验证 | 当前 127 项自动化测试 |
...
| 需求 | 产品目标和范围 | PRD、25 个 FR、10 个 NFR、17 个 UC | 需求可测试、范围外明确 | 已完成并受控 |
| 设计 | 需求/用例基线 | 总体设计、UI 原型、数据流、状态机和安全边界 | 模块和契约可映射到源码 | 已完成并受控 |
| 编码 | 架构与接口 | Agent、ModelClient、工具、桌面服务、Renderer | 核心闭环实现，相关测试通过 | 核心完成 |
| 验证 | 候选构建 | 127 项测试、Windows 自动化门禁、真实模型、桌面检查、交付脚本 | 无阻塞缺陷、演示稳定 | 进行中 |
