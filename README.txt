...
特色功能：左侧显示过滤后的项目结构和变更文件；中间只读预览代码及任务前后双栏 Diff；右侧展示 Agent 时间线、输入和停止操作。支持文件列表、搜索、读取、精确替换、写入；命令逐次审批并具有超时、输出截断和取消能力；所有路径限制在所选项目内；API Key 使用系统安全存储加密且不会传给命令子进程；任务历史按项目保存在本机；模型协议损坏时明确失败；最终结果区分已验证与未验证内容。
