...
- 列表、搜索、读取、精确替换、带文件哈希保护的按行编辑和完整写入工作区文件；工具失败返回稳定错误码、可重试标记与恢复建议。
- 对每条本地命令显示原因、命令和工作目录，并由用户逐次批准。
- 支持命令超时、输出截断、任务取消和最大步骤限制；剩余三轮时要求模型优先验证和收尾，重复读取/搜索或同类失败形成无进展循环时先纠偏、再次发生则提前停止；在 Windows 上停止命令时清理整棵子进程树，并区分人工批准等待与真实执行耗时。
- 正式计划模式通过计划状态机限制副作用；直接模式在文件修改后也会要求至少重新读取最新内容或运行相关命令，无法验证时必须在最终答复中如实说明。
