...
- 实时展示模型生成内容，并展示工具结果、运行输出、变更文件及任务前后双栏 Diff。
...
开发冻结前可运行 `pnpm run verify` 完成类型检查、123 项测试和构建，再运行 `pnpm run verify:delivery` 检查 README.txt 长度、Git 历史凭据形态、禁入文件、文档链接和演示故障基线。
